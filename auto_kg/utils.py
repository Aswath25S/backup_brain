...
# image OCR (png/jpg/jpeg)
    for ext in ['png', 'jpg', 'jpeg']:
        img_files, sub_paths = find_files(directory, filetype=ext)
        for img_file, sub_path in zip(img_files, sub_paths):
            try:
                ...
                if text.strip():
                    raw_texts.append(text)
                    raw_sources.append(img_file)
            except Exception as e:
                print("Failed to process image:", sub_path, img_file)
                print("Error:", e)
    ...
    # segment
    texts, sources = split_texts_with_source(processed_texts, raw_sources, chunk_size=chunk_size, separator=separator)
