...
if __name__ == "__main__":
    # === Step 1: Load data ===
    texts, sources = load_and_process_files("raw_data")

    # === Step 2: Initialize AutoKG ===
    kg = autoKG(
        texts=texts,
        source=sources,
        ...
        main_topic="your_main_topic"  # 👈 replace with actual topic like "LLMs and KGs"
    )

    # === Step 3: Extract keywords via clustering ===
    keywords, tokens_used = kg.cluster(n_clusters=15, verbose=True)
    print("Extracted Keywords:", keywords)
    print("Total tokens used:", tokens_used)

    # === Step 4: Create knowledge graph matrices ===
    kg.coretexts_seg_individual(trust_num=5, seg_mtd="laplace")

    # === Step 5: Run KG-based semantic search ===
    #query = "How are LLMs used in knowledge graph construction?"
    query = "Tell me about my trip at washinton dc."
    record = kg.KG_prompt(query, use_u=True)

    # === Step 6: Generate a completion using the KG context ===
    response, keywords_info, ref_info, _ = kg.completion_from_record(record, show_prompt=True)
    print("\n=== Generated Answer ===\n", response)

    # === Step 7: Visualize the knowledge graph ===
    kg.draw_graph_from_record(record, save_fig=True, save_path="knowledge_graph.png")